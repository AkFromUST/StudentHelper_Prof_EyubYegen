#!/usr/bin/env python3
"""
OGE Email Processor - Duplicates Version
...
**KEY DIFFERENCE from email_processor.py:**
This version ALLOWS duplicate filenames by appending a counter.
- If file.pdf exists, saves as file_1.pdf
- If file_1.pdf exists, saves as file_2.pdf
- And so on...

This is useful when OGE has multiple different documents with the same name.
"""
...
UNMATCHED_CSV = "unmatched_documents_duplicates.csv"


class EmailProcessorDuplicates:
    """Processes OGE emails and organizes attachments (allows duplicates)."""
    ...
    def __init__(self):
        ...
        self.duplicate_count = 0  # Files saved with duplicate names
    ...
    def log(self, message: str, level: str = "info"):
        ...
        icons = {"info": "ℹ️", "success": "✅", "warning": "⚠️", "error": "❌", "start": "🚀", "download": "📥", "duplicate": "🔄"}
    ...
    def get_unique_filename(self, target_dir: Path, filename: str) -> str:
        ...
        Examples:
        - file.pdf -> file.pdf (if doesn't exist)
        - file.pdf -> file_1.pdf (if file.pdf exists)
        - file.pdf -> file_2.pdf (if file.pdf and file_1.pdf exist)
        ...
        Returns:
            Unique filename with counter appended if necessary
        """
    ...
    def save_attachment(self, part, target_dir: Path, filename: str) -> Tuple[bool, str, str]:
        """Save an email attachment to the target directory.
        
        Handles duplicates by appending a counter to the filename.
        
        Returns:
            Tuple of (success: bool, status: str, saved_filename: str)
            - status can be: "saved", "saved_duplicate", "error"
            - saved_filename is the actual filename used (may differ from input if duplicate)
        """
        try:
            ...
            is_duplicate = (unique_filename != filename)
            ...
            if payload:
                with open(file_path, 'wb') as f:
                    f.write(payload)
                status = "saved_duplicate" if is_duplicate else "saved"
                return (True, status, unique_filename)
    ...
    def process_email(self, mail: imaplib.IMAP4_SSL, email_id: bytes) -> int:
        ...
        try:
            ...
            for part in msg.walk():
                ...
                # Save attachment (with duplicate handling)
                success, status, saved_filename = self.save_attachment(part, target_dir, filename)
                
                if success:
                    if status == "saved_duplicate":
                        downloaded += 1
                        ...
                        if matched_name != "Unmatched":
                            ...
                            self.matched_people[matched_name].append(saved_filename)
                            self.log(f"🔄 DUPLICATE: {filename} -> {saved_filename} ({matched_name})", "duplicate")
                        else:
                            self.unmatched_count += 1
                            self.unmatched_documents.append(saved_filename)
                            self.log(f"🔄 DUPLICATE (Unmatched): {filename} -> {saved_filename}", "duplicate")
                    elif matched_name == "Unmatched":
                        downloaded += 1
                        self.unmatched_count += 1
                        self.unmatched_documents.append(saved_filename)
                        self.log(f"📁 UNMATCHED: {saved_filename} -> {target_dir}", "warning")
                    else:
                        downloaded += 1
                        self.processed_count += 1
                        ...
                        if matched_name not in self.matched_people:
                            self.matched_people[matched_name] = []
                        self.matched_people[matched_name].append(saved_filename)
                        self.log(f"📥 Saved: {saved_filename} -> {matched_name} (Page {self.mapping.get(matched_name, '?')})", "download")
    ...
    def run(self, unread_only: bool = True, mark_as_read: bool = False):
        ...
        self.log("=== OGE Email Processor (Duplicates Allowed) ===", "start")
...
def main():
    ...
    import argparse
    parser = argparse.ArgumentParser(description='OGE Email Processor - Duplicates Version')
    ...
    print("=" * 60)
    print("OGE Email Processor - Duplicates Version")
    print("=" * 60)
    ...
    print()
    print("⚠️  DUPLICATE HANDLING: This version allows duplicate filenames")
    print("   Files with same name will be saved as: file.pdf, file_1.pdf, file_2.pdf, etc.")
    print("=" * 60)
    print()
    ...
    print("   4. Allow duplicate filenames (append counter)")
    print("   5. Organize files by Page/PersonName")
    print("   6. Put unmatched files in _Unmatched folder")
    print()
    ...
    try:
        processor = EmailProcessorDuplicates()
